...
if  (isset($_GET['status'])) {
    ...
    $result = mysql_query("UPDATE aktuator SET status= '$status' WHERE id='1'");

	// Check for succesfull execution of query and no results found
    if ($result) {
        // successfully updation of aktuator status (status)
        $response = "kipas angin Status successfully updated.";
    ...
    $response = "Parameter(s) are missing. Please check the request";
